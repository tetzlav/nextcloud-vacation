...
- Lets calendar admins assign one personal approver per employee from the configured admin groups. The assignment controls notification routing only; every calendar admin keeps the existing management and approval permissions.
...
The script's `--upgrade` option is deliberately app-scoped: it bootstraps
Nextcloud's CLI environment and invokes the update process only for
`nextcloud_vacation`. This also works while Nextcloud exposes only its reduced
command set because an app upgrade is pending. It never runs the global `occ
upgrade`, so unrelated App Store applications are not updated. Ordinary code,
template, CSS or translation changes need neither `--upgrade` nor `--migrate`.
...
the selected year. Amount corrections are recorded as new negative entries. A reason can
be corrected directly on its entry; internally this creates a linked zero-day journal
entry, so the original booking remains unchanged. Entries are never edited or deleted.
Special leave is not carried into another year automatically.
...
notifications are enabled, postings and reason corrections queue localized status emails.
...
Calendar admins can expand an employee's name in **Vacation management** and assign a
personal approver from the configured admin groups. When an assignment exists, approver
notifications for new requests, missing approved calendar entries and confirmed
cancellations are sent only to that user. Without an assignment, the global
`approver_users` configuration remains the default; if it is empty, all eligible members
of the configured admin groups remain the fallback. The assignment does not restrict who
may open the management overview or perform approval actions.
...
from the beginning.
When a duplicate conflict is first detected, the employee receives one localized warning
through the asynchronous mail queue if employee notifications are enabled. Repeated
synchronization does not send the warning again.
